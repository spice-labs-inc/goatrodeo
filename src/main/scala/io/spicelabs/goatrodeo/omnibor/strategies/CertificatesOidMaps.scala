...
/** Phase-7 second-pass remediation: extracted out of `Certificates.scala`
  * to bring the parent file under the inv #9 token limit.
  *
  * All the canonical-mapping tables used by the Certificates strategy
  * across Phases 3-7 live here. They are PURE DATA — no logic, no
  * side effects. Centralizing them in one file makes it easy to
  * verify the inventory matches `certificates-strategy/appendices.md`
  * §A (qualifier reference).
  *
  * The maps are exposed as `private[strategies]` so both the
  * `Certificates` companion object (in `Certificates.scala`) and the
  * `CertificatesState` class (in `CertificatesState.scala`) can use
  * them.
  *
  * Refactor record:
  * - 2026-05-04: extracted from Certificates.scala (Phase 7 second-pass
  *   gap A1). Pure relocation; no semantic change.
  */
private[strategies] object CertificatesOidMaps {

  // ---------- X.509 algorithm + signature OIDs (Phase 3-4) -----------

  /** Public-key OIDs → (canonical alg, optional params token). */
  private[strategies] val pubkeyOidMap: Map[String, (String, Option[String])] = Map(
  ...
  // ---------- Phase 7 PKCS#8 / PBES2 + PBES1 OIDs --------------------
