...
class RodeoHost extends APIFactoryReceiver, APIFactorySource {
    ...
    def exportImport() = {
        ...
        log.debug(s"exported ${publishedAPIS.size} in ${exportTime/1000} milliseconds")
    ...
    def isActiveComponent(component: RodeoComponent) = activeComponents.contains(component)
    def isInactiveComponent(component: RodeoComponent) = inactiveComponents.contains(component)
    ...
    def discoverComponents(): Iterator[RodeoComponent] = {
        ...
        val (loader, time) = elapsedTime { ServiceLoader.load(classOf[RodeoComponent]) }
    ...
    private def isComponentValid(component: RodeoComponent): Boolean = {
        ...
        val initializedOK = Try {
            ...
            case Failure(exception) =>
                log.error(s"Component of type ${component.getClass().toGenericString()} failed to initialize")
                false
        ...
        val validIdentity = isIdentityValid(component) match {
            case Some(error) =>
                log.error(s"Component of type ${component.getClass().toGenericString()} $error")
                false
            case None => true
        }

        // for versioning, we need to think about what is considered valid
        val validVersion = component.getComponentVersion() == RodeoEnvironment.currentVersion()
    ...
    private def isIdentityValid(component: RodeoComponent): Option[String] = {
        ...
        val whatsWrong = (Option(identity.publisher()), Option(identity.name())) match {
    ...
    override def publishFactory[T <: API](
        ...
        apiType: Class[T]): Unit = {
            ...
            if (isLoggedArgError(identity, nameOf(publisher.getIdentity), thisMethod, log))
                return
    ...
    def addAPI(apiName: String, theAPI: PublishedAPI): Unit = {
        ...
        publishedAPIS.get(apiName) match {
            case Some(existingAPI) =>
                log.error(s"Component $componentName attempted to publish the API $apiName of type $apiType but it has already been published by ${existingAPI.publisher.getIdentity().name()}")
            case None =>
                log.debug(s"publishing API $apiName of type $apiType from $componentName")
    ...
    def removeAPI(apiName: String): Unit = {
        publishedAPIS.remove(apiName) match {
            case Some(api) =>
                log.debug(s"Removed $apiName of type ${api.apiType.toString()} published by ${api.publisher.getIdentity().name()}")
            case None =>
    ...
    override def getAPIFactory[T <: API](
        ...
        factoryType: Class[T]): ju.Optional[APIFactory[T]] = {
            import RodeoHost.isLoggedArgError
            ...
            publishedAPIS.get(name) match {
                case Some(publishedAPI) => 
                    ...
                    then {
                        log.error(s"Component $componentName attempted to get API $name of type $apiType but the actual type was ${publishedAPI.apiType.toString()}")
                        emptyReturn
                    }
                    else {
                        // TODO - check to see if we're already subscribed
                        log.debug(s"Component $componentName subscribed to API $name of type $apiType")
                        updateAPI(name, publishedAPI, subscriber)
                        ju.Optional.of[APIFactory[T]](publishedAPI.apiFactory.asInstanceOf[APIFactory[T]])
                    }
                case None =>
                    log.debug(s"Component $componentName attempted to subscribe to API $name, but there is no API by that name")
                    emptyReturn
            }
    }
...
object RodeoHost {
    def argCheck[T](arg: T, argName: String, methodName: String): Either[T, String] = {
        Option(arg) match {
    ...
    def isLoggedArgError[T](arg: T, argName: String, methodName: String, log: Logger): Boolean = {
