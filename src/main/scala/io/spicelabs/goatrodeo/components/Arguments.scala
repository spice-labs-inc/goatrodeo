...
/** This is the implementation of the command line argument API for components.
  ...
  * Command line argument listeners will be called after import and before
  * onLoadingComplete. Therefore component listeners should register during the
  * import step.
  *
  * @param name
  *   The name of the component that wants to listen to arguments
  */
...
object Arguments {
  ...
  /** Adds an argument listener to the list of existing listeners.
    *
    * @param name
    *   the name of the component listening
    * @param listener
    *   an interface to call with command line arguments
    */
  def addListener(name: String, listener: RodeoArgumentListener): Unit = {
    if (listener == null) {
      logger.warn(
        s"Component $name attempting to register null listener; ignoring"
      )
      return
    }
    listeners.synchronized {
      if (listeners.exists((n, l) => n == name)) {
        logger.warn(
          s"Component $name attempting to register duplicate argument listener; ignoring"
        )
      } else {
  ...
  def processComponentArguments(
      arguments: Map[String, Vector[Array[String]]]
  ): Unit = {
    listeners.synchronized {
  ...
  private def processArguments(
      name: String,
      listener: RodeoArgumentListener,
      args: Option[Vector[Array[String]]]
  ): Unit = {
    ...
    args.foreach(argList => {
      argList.foreach(args => {
        ...
        if (result.hasError()) {
          logger.error(
            s"Component $name reported and error processing '${args.mkString}': ${result.getError().get()}"
          )
        } else {
    ...
    if (finalResult.hasError()) {
      logger.error(
        s"Component $name reported an error finishing argument processing: ${finalResult.getError().get()}"
      )
    }
  }
  ...
  def addArgs(
      name: String,
      newArgs: Array[String],
      argsSoFar: Map[String, Vector[Array[String]]]
  ): Map[String, Vector[Array[String]]] = {
