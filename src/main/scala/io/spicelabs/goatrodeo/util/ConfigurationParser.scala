...
object ConfigurationParser {
  ...
  lazy val parser: OParser[Unit, Configuration] = {
    ...
    OParser.sequence(
      ...
      opt[File]('b', "build")
        // Repeatable: one occurrence per build directory, appended below.
        .unbounded()
      ...
      opt[File]("ignore")
        // Repeatable: one occurrence per ignore file, appended below.
        .unbounded()
        .text(
      ...
      opt[File]("file-list")
        // Repeatable: one occurrence per list file, appended below.
        .unbounded()
        .text(
      ...
      opt[String]("exclude-pattern")
        // Repeatable: one occurrence per exclusion pattern. Without unbounded,
        // scopt rejects a second occurrence as an unknown option and its value
        // as a stray argument, aborting the run.
        .unbounded()
        .text(
      ...
      opt[String]("mime-filter")
        // Repeatable: one occurrence per filter (the help text lists one
        // include/exclude form per line).
        .unbounded()
        .text(
      ...
      opt[File]("mime-filter-file")
        // Repeatable: one occurrence per filter file, appended below.
        .unbounded()
