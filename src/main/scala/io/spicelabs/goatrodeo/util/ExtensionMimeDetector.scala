...
/** Extension-and-magic-byte short-circuit for MIME type detection.
  *
  * For well-known file types where the extension plus a few magic bytes are
  * sufficient to determine the MIME type, this detector bypasses the
  * (expensive) Tika content-based detection entirely. The rest of the pipeline
  * (augmenter chain, `notArchive` checks, etc.) runs unchanged.
  *
  * Currently handles:
  ...
  * `None`, allowing Tika to perform full content-based detection as a fallback.
  */
object ExtensionMimeDetector {

  /** Java class file magic: 0xCAFEBABE */
  private val ClassMagic =
    Array(0xca.toByte, 0xfe.toByte, 0xba.toByte, 0xbe.toByte)

  /** ZIP/JAR magic: PK\x03\x04 */
  private val ZipMagic =
    Array(0x50.toByte, 0x4b.toByte, 0x03.toByte, 0x04.toByte)

  /** Read the first 4 bytes of the stream and compare against `expected`.
    *
    * Returns `false` on any I/O error or if fewer than 4 bytes are available.
    */
  ...
  /** Attempt to determine the MIME type from extension + magic bytes.
    ...
    * @return
    *   `Some(mimeType)` if the extension and magic bytes match a known type;
    *   `None` to fall through to Tika
    */
  def detect(artifact: ArtifactWrapper): Option[String] = {
    ...
    if (lower.endsWith(".class")) {
      ...
      JavaArchiveDetector.javaArchiveExtensions
        .exists(ext => lower.endsWith(ext))
    ) {
