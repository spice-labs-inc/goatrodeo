...
/** The ambient process state Goat Rodeo is running in, captured once rather
  ...
  *
  * WHY: reading `System.getProperty("user.home")` (or the working directory, or
  * the environment) from deep inside processing code makes the set of inputs
  * undiscoverable and the code untestable. Capturing it here means
  * [[Configuration]] is the single answer to "what is this run configured
  * with", and a test can supply a different environment without touching global
  * JVM state.
  */
...
object RuntimeEnvironment {
  ...
  lazy val default: RuntimeEnvironment = RuntimeEnvironment(
    ...
    properties = System.getProperties().asScala.toMap
  )
}
...
/** The complete configuration for a Goat Rodeo run.
  ...
  *   are dropped too. Set only via `--expiry` or
  *   [[io.spicelabs.goatrodeo.GoatRodeoBuilder.withExpiry]] — there is
