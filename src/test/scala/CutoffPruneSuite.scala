...
class CutoffPruneSuite extends munit.FunSuite {
  ...
  // Asserts against `ConfigurationParser.parseCutoff` rather than setting the
  // `goatrodeo.expiry` system property. The parsing rules are what this test is
  // actually about.
  test("parseCutoff parses epoch millis and ISO, and is empty when blank") {
    assertEquals(ConfigurationParser.parseCutoff(""), None)
    assertEquals(ConfigurationParser.parseCutoff("   "), None)

    val millis = Instant.parse("2026-01-01T00:00:00Z").toEpochMilli()
    assertEquals(
      ConfigurationParser.parseCutoff(millis.toString).map(_.toEpochMilli()),
      Some(millis)
    )

    assertEquals(
      ConfigurationParser.parseCutoff("2026-01-01T00:00:00Z").map(_.toString),
      Some("2026-01-01T00:00:00Z")
    )
  }

  test("--cutoff is the only route to a cutoff on the command line") {
    val parsed = ConfigurationParser.parse(Array("--cutoff", "2026-01-01"))
    assertEquals(
      parsed.flatMap(_.cutoff).map(_.toString),
      Some("2026-01-01T00:00:00Z")
    )
  }
  ...
  /** WHY: the cutoff used to have a second, ambient source — the
    ...
    * via `--cutoff` or `withCutoff`. This guards against reintroducing it.
    *
    ...
    * scheduled.
    */
  test("no system property can set the cutoff") {
    val prop = "goatrodeo.expiry"
    val saved = Option(System.getProperty(prop))
    try {
      System.setProperty(prop, "2026-01-01T00:00:00Z")
      assertEquals(ConfigurationParser.parse(Array()).flatMap(_.cutoff), None)
      assertEquals(Configuration().cutoff, None)
    } finally
      saved match {
