...
/** Phase 8 — coverage matrix completeness suite.
  ...
  * represented. Fail the suite if any required > cell in the coverage matrix is
  * unfilled.
  *
  * > A failing coverage test is a test-data gap, not a strategy bug > — but it
  * still fails the build. This prevents silent erosion of > test coverage as
  * fixtures get added/removed.
  *
  * Strategy: parse every sidecar, extract qualifiers from declared pURLs (and
  * MIMEs / metadata where relevant), build sets of (alg, size), (alg, curve),
...
class CertificatesCoverageSuite extends FunSuite {
  ...
  private def parseSidecar(file: File): Sidecar = {
    ...
    def getStringMap(jval: JValue): Map[String, String] = jval match {
      case JObject(fields) =>
        fields.collect { case (k, JString(v)) =>
          k -> v
        }.toMap
      case _ => Map.empty
    }
  ...
  test("[COVERAGE] X.509 sig-alg: at least sha1-rsa, sha256-rsa, ed25519") {
    val sigAlgs = purlsByScheme
      .getOrElse("x509", Vector.empty)
      .flatMap(_.get("sig-alg"))
      .toSet
  ...
  test("[COVERAGE] X.509 self-signed: both true and false") {
    val selfSignedValues = purlsByScheme
      .getOrElse("x509", Vector.empty)
      .flatMap(_.get("self-signed"))
      .toSet
  ...
  test("[COVERAGE] Encoding: at least one PEM and one DER fixture") {
    ...
    assert(
      mimes.contains("application/x-pem-file") || mimes.contains(
        "application/x-pem-bundle"
      ),
      s"PEM encoding not present in any fixture's mustContain (observed: $mimes)"
    )
    assert(
      mimes.contains("application/pkix-cert") || mimes.contains(
  ...
  test(
    "[COVERAGE] PEM bundle size: at least one single-cert and one multi-cert (≥ 10 entries)"
  ) {
    val bundleSidecars =
      sidecars.filter(_.mimeContains.contains("application/x-pem-bundle"))
    assert(bundleSidecars.nonEmpty, "no pem-bundle fixture found")
    val purlCounts = bundleSidecars.map(_.purlsContains.length)
    val singleish =
      purlCounts.exists(c => c >= 1 && c <= 4) // ≤ 2 certs each emits ≤ 2 pURLs
    val largeBundle =
      purlCounts.exists(_ >= 20) // 10 certs ≈ 20 pURLs (each cert: spki + cert)
    assert(
      singleish,
      s"no small-bundle fixture (1-2 certs); pURL counts: $purlCounts"
    )
    assert(
      largeBundle,
      s"no large-bundle fixture (≥10 certs); pURL counts: $purlCounts"
    )
  }
  ...
  test(
    ...
    val sshCerts = purlsByScheme
      .getOrElse("ssh", Vector.empty)
      .filter(p =>
        p.contains("cert-type") && p.contains("alg") && p.contains("sig-alg")
      )
  ...
  test("[COVERAGE] PGP alg: rsa, ed25519") {
    val algs = purlsByScheme
      .getOrElse("pgp", Vector.empty)
      .flatMap(_.get("alg"))
      .toSet
  ...
  test("[COVERAGE] PGP version: v4 and v6") {
    val versions = purlsByScheme
      .getOrElse("pgp", Vector.empty)
      .flatMap(_.get("version"))
      .toSet
  ...
  test(
    "[COVERAGE] CRL sig-alg: at least one SHA-1 (sha1-rsa) and one modern (sha256-rsa or ed25519)"
  ) {
    val crlPurls = purlsByScheme
      .getOrElse("x509", Vector.empty)
    ...
    val crlSigAlgs = sidecars
      ...
      .flatMap(parsePurl)
      .flatMap(_._2.get("sig-alg"))
      .toSet
    val hasSha1 = crlSigAlgs.exists(_.contains("sha1"))
    val hasModern =
      crlSigAlgs.exists(s => s.contains("sha256") || s == "ed25519")
  ...
  test(
    "[COVERAGE] Private key envelope — unlocked: plaintext with rsa AND with ed25519"
  ) {
    val unlocked = sidecars
      .filter(
        _.metadataContains.get("Certificates:Envelope").contains("plaintext")
      )
    val rsaOk = unlocked.exists(
      _.metadataContains.get("Certificates:KeyAlgorithm").contains("rsa")
    )
    val edOk = unlocked.exists(
      _.metadataContains.get("Certificates:KeyAlgorithm").contains("ed25519")
    )
  ...
  test(
    "[COVERAGE] Private key envelope — encrypted: pkcs8-encrypted AND openssh-encrypted"
  ) {
    val envelopes =
  ...
  test(
    ...
    val envelopes =
      sidecars.flatMap(_.metadataContains.get("Certificates:Envelope")).toSet
    assert(
      envelopes.contains("pem-legacy-encrypted"),
  ...
  test(
    ...
    val envelopes =
      sidecars.flatMap(_.metadataContains.get("Certificates:Envelope")).toSet
    assert(
      envelopes.contains("pgp-encrypted-secret-key"),
      s"pgp-encrypted-secret-key envelope not present in any fixture (observed: $envelopes)"
    )
  }

  test(
    "[COVERAGE] Private key envelope — plaintext PGP (Phase-7 second-pass: pgp-secret-* unencrypted fixture)"
  ) {
  ...
  test(
    "[COVERAGE] Keystore encryption state: at least one unencrypted AND one encrypted"
  ) {
    ...
    val encrypted = keystoreSidecars.exists { s =>
      s.metadataContains
        .get("Certificates:KeystoreEncrypted")
        .contains("true") ||
    ...
    assert(
      unencrypted,
      "no unencrypted (loadable, EntryCount > 0) keystore fixture"
    )
  }
}
