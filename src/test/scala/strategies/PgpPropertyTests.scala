...
/** Property-based tests for the PGP path.
  ...
  *
  * Phase 6 first-pass gap analysis G6 + second-pass N5/N9: example-based
  * tests can only pin specific fixtures. These properties hold across
  * the full corpus and their **strength** is the focus of N5: each
  * property must catch a realistic class of bug, not be tautological.
  *
  * ### Properties (post-N5 / N9 strengthening)
  *
  *   1. *Fingerprint hex shape* — every parsed key's fingerprint is
  *      lowercase hex, length ∈ {40, 64} (v4=20B SHA-1, v5/v6=32B
  *      SHA-256). Catches case bugs and length-mapping bugs.
  *   2. *Parse idempotence (FULL key equality, not just fingerprint
  *      set)* — parsing the same file twice yields identical PgpKey
  *      values across all fields (fingerprint, version, alg id,
  *      canonical alg, keySize, curve, isPrimary, dates, userIds).
  *      Strengthened from G6's original "fingerprint set only" form.
  *      Catches any non-deterministic decode of algorithm/curve/size/
  *      timestamps between parses.
  *   3. *pURL canonical-form structural invariant* — every emitted
  *      pURL parses to a string that:
  *        - starts with `pkg:pgp/fingerprint@`
  *        - contains the lowercase fingerprint hex
  *        - has alphabetically-sorted qualifiers
  *        - includes both `alg=` and `version=` qualifiers
  *      Strengthened from G6's `once == twice` form (which only proved
  *      function purity).
  *   4. *Per-key pURL distinctness* — every key in a ring emits a
  *      pURL that is distinct after canonicalization. Same dedup the
  *      strategy applies in `getPurls`.
  *   5. *Emission-path equivalence (N9)* — the property-test pURL
  *      computation matches what the actual `getPurls` emission path
  *      produces (the new property exercises the strategy's claim
  *      pipeline through `Certificates.computeCertificateFiles`-style
  *      classification, so it's not just a unit test of the helper).
  *
  * ## Why this matters
  *
  * G6 in `info/2026_05_01_phase6_gaps.md` flagged the absence of
  * property tests; N5/N9 in the second-pass gap analysis flagged that
  * the original property set was weak (`once == twice` is referential
  * transparency, not a meaningful invariant; "fingerprint set" missed
  * algorithm/version/curve mismatches). These reformulated properties
  * actually catch realistic bug classes.
  */
class PgpPropertyTests extends ScalaCheckSuite {

  /** All `.asc` PGP fixtures in the corpus. The property tests run
    * against every one. */
  ...
  property("[PROP] every parsed PGP key has lowercase-hex fingerprint of length 40 or 64 (G6)") {
    forAll(genFixture) { f =>
      parse(f) match {
  ...
  property("[PROP] parse idempotence: same file → identical PgpKey list (N5 / G6)") {
    forAll(genFixture) { f =>
      val a = parse(f)
      val b = parse(f)
      (a, b) match {
  ...
  // distinct keys).
  property("[PROP] purlForPgpKey: one canonical pURL per cryptographic identity (G6)") {
    forAll(genFixture) { f =>
      parse(f) match {
        case None => true
        case Some(r) =>
          val purls = r.keys.map(k =>
            Certificates.purlForPgpKey(k).canonicalize().nn
          )
  ...
  /** Minimal `Item` for plumbing `state.getPurls` — the PGP branch
    * doesn't actually read the Item, but the signature requires one. */
  ...
  // unchanged.
  property("[PROP] strategy emission path (CertificatesState.getPurls) matches unit computation (N9)") {
    forAll(genFixture) { f =>
      parse(f) match {
        case None => true
        case Some(r) =>
