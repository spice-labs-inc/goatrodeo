  ...
  property("[PROP] pURL canonical form: structural invariants (N5 / G6)") {
    forAll(genFixture) { f =>
      parse(f) match {
        ...
        case Some(r) =>
          r.keys.forall { k =>
            val purl = Certificates.purlForPgpKey(k).toCanonical()
  ...
  property(
    ...
    forAll(genFixture) { f =>
      parse(f) match {
        ...
        case Some(r) =>
          val purls =
            r.keys.map(k => Certificates.purlForPgpKey(k).toCanonical())
    ...
    forAll(genFixture) { f =>
      parse(f) match {
        ...
        case Some(r) =>
          ...
          val (purlSet, _) = state.getPurls(w, stubItem(), SingleMarker())
          val emittedPurls = purlSet.canonicalStrings
          val emittedSet = emittedPurls.toSet
          val unitSet = r.keys
            .map(k => Certificates.purlForPgpKey(k).toCanonical())
            .toSet
          emittedSet == unitSet
      }
    }
