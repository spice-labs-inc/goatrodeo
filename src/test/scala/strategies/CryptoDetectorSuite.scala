...
/** Phase 2 — `CryptoDetector` content-sniff signature unit tests.
  ...
  *
  * Test name pattern: `[<row>]: <description>` so a reviewer auditing the
  * signature table can grep `[PEM cert]:` and see every test that traces to
  * that row.
  *
  * ## Test discipline
  *
  * Per HS-4 (test corpus means the actual test corpus), this suite also fans
  * out across a sample of real fixture bytes from `test_data/certificates/` to
...
class CryptoDetectorSuite extends FunSuite {
  ...
  test(
    "[INVARIANT] augmenter never strips text/* MIMEs (contrast SaffronDetector)"
  ) {
    val input = Set("text/plain", "text/html")
    val out = augment("-----BEGIN CERTIFICATE-----\n".getBytes, input, "x.pem")
    assert(input.subsetOf(out), s"text-prefixed MIMEs were stripped: out=$out")
  }
  ...
  test(
    "[PEM cert]: BEGIN CERTIFICATE adds application/x-pem-file + application/x-x509-ca-cert"
  ) {
    val out = detect(
      "-----BEGIN CERTIFICATE-----\nMIID...\n-----END CERTIFICATE-----\n".getBytes
    )
  ...
  test(
    "[PEM bundle]: multiple BEGIN CERTIFICATE adds application/x-pem-bundle"
  ) {
    val data =
      "-----BEGIN CERTIFICATE-----\nA\n-----END CERTIFICATE-----\n" +
        "-----BEGIN CERTIFICATE-----\nB\n-----END CERTIFICATE-----\n"
  ...
  test(
    "[PEM RSA private]: BEGIN RSA PRIVATE KEY adds application/x-pem-private-key"
  ) {
  ...
  test(
    "[PEM EC private]: BEGIN EC PRIVATE KEY adds application/x-pem-private-key"
  ) {
  ...
  test(
    "[PEM generic private]: BEGIN PRIVATE KEY adds application/x-pem-private-key"
  ) {
  ...
  test(
    "[PEM encrypted private]: BEGIN ENCRYPTED PRIVATE KEY adds application/x-pem-encrypted-private-key"
  ) {
  ...
  test(
    "[OpenSSH private]: BEGIN OPENSSH PRIVATE KEY adds application/x-openssh-private-key"
  ) {
  ...
  test(
    "[OpenSSH pubkey]: ssh-rsa first token adds application/x-openssh-public-key"
  ) {
  ...
  test(
    "[OpenSSH pubkey]: ssh-ed25519 first token adds application/x-openssh-public-key"
  ) {
  ...
  test(
    "[OpenSSH pubkey]: ecdsa-sha2-nistp256 first token adds application/x-openssh-public-key"
  ) {
  ...
  test(
    "[OpenSSH pubkey]: sk-ssh-ed25519@openssh.com first token adds application/x-openssh-public-key"
  ) {
  ...
  test(
    "[OpenSSH cert]: ssh-ed25519-cert-v01@openssh.com first token adds application/x-openssh-certificate"
  ) {
  ...
  test(
    "[OpenSSH cert]: ssh-rsa-cert-v01@openssh.com first token adds application/x-openssh-certificate"
  ) {
  ...
  test(
    ...
    assert(
      !out.contains("application/x-openssh-certificate"),
      s"placeholder '[email protected]' string should not match a cert token; got $out"
    )
  }

  test(
    "[PGP armored pub]: BEGIN PGP PUBLIC KEY BLOCK adds application/pgp-keys"
  ) {
  ...
  test(
    "[PGP armored priv]: BEGIN PGP PRIVATE KEY BLOCK adds application/pgp-keys"
  ) {
  ...
  test(
    "[PGP binary]: first byte 0xC6 (new-format public-key packet) adds application/pgp-keys"
  ) {
  ...
  test(
    "[PGP binary]: first byte 0x98 (old-format public-key packet) adds application/pgp-keys"
  ) {
  ...
  test("[JKS]: 0xfe 0xed 0xfe 0xed magic adds application/x-java-keystore") {
    val out = detect(
      Array[Byte](0xfe.toByte, 0xed.toByte, 0xfe.toByte, 0xed.toByte, 0, 0)
    )
    assert(out.contains("application/x-java-keystore"))
  }

  test(
    "[JCEKS]: 0xce 0xce 0xce 0xce magic adds application/x-java-jce-keystore"
  ) {
    val out = detect(
      Array[Byte](0xce.toByte, 0xce.toByte, 0xce.toByte, 0xce.toByte, 0, 0)
    )
  ...
  test(
    "[DER X.509]: real DER cert (Mozilla CA via fixture) adds application/pkix-cert"
  ) {
    ...
    assert(
      out.contains("application/pkix-cert"),
      s"DER X.509 cert should detect application/pkix-cert; got $out"
    )
  }

  test(
    "[DER CRL]: real DER CRL (synthetic fixture) adds application/pkix-crl"
  ) {
    ...
    assert(
      out.contains("application/pkix-crl"),
  ...
  test("[NEG]: PEM with typo (BEGIN CERTIFICAT) does NOT match") {
    ...
    assert(
      !out.contains("application/x-x509-ca-cert"),
      s"typo'd PEM header must not match; got $out"
    )
    assert(
      !out.contains("application/x-pem-file"),
      s"typo'd PEM header must not match; got $out"
    )
  }

  test(
    "[NEG]: 0x30 0x82 prefix that is NOT a valid X.509 cert and NOT .p12/.pfx returns empty"
  ) {
    // 0x30 0x82 followed by garbage — too short / structurally broken
    // for ASN.1 X.509 or CRL. No extension hint either.
    val out = detect(
      Array[Byte](0x30, 0x82.toByte, 0x00, 0x10) ++ Array.fill[Byte](18)(0x00),
      name = "garbage.bin"
    )
    assert(
      !out.contains("application/pkix-cert"),
    ...
    assert(
      !out.contains("application/pkcs12"),
  ...
  test("[FIXTURE] Mozilla bundle PEM is detected as application/x-pem-bundle") {
    ...
    assert(
      out.contains("application/x-pem-bundle"),
  ...
  test(
    "[FIXTURE] PGP armored key from synthetic generator detected as application/pgp-keys"
  ) {
    val sample = new File(
  ...
  test("[FIXTURE] JKS keystore detected as application/x-java-keystore") {
    ...
    assert(
      out.contains("application/x-java-keystore"),
  ...
  test(
    ...
    assert(
      out.contains("application/pkcs7-mime"),
