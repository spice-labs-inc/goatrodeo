...
/** Phase 9 — mechanical claims-citation + internal-link checker.
  ...
  *
  * > - All doc updates land > - Internal links are valid (no 404 links) > -
  * ADRs exist with both human and LLM sections per invariant #14 > - Every
  * claim in `info/certificates_strategy.md` and its `_llm` > copy cites a named
  * test
  *
  * And §"Phase Exit Review (HS-2)":
  *
  * > Claims-verification (step 2) is especially important for this > phase:
  * read every documentation claim, identify its referenced > test, run that
  * test, confirm the test actually tests what the > claim says — not just that
  * a test with that name exists.
  *
  * This suite is the MECHANICAL portion of that requirement: it verifies
  * test-name references exist as compilable test names AND internal markdown
  * links resolve to real files. The semantic verification ("does the test
  * actually test the claim?") still needs human reading — but a missing test
  * name or 404 link is caught here.
  */
class CertificatesDocCitationTests extends FunSuite {
  ...
  /** Extract every `test("name") {` and `property("name") {` declaration from a
    * Scala test file.
    */
  ...
  /** Test-name patterns the Phase 9 doc uses for citations:
    ...
    *   - `\`SuiteName.test_name\`` (alternate format used in some Phase-0
    *     tests)
    *   - `\`[PROP] ...\`` / `\`[LEAK SWEEP] ...\`` (bare test-name with
    *     bracketed tag)
    */
  private def extractTestRefs(content: String): Set[String] = {
    ...
    val scopedTestNames =
      rxScopedTest.findAllMatchIn(content).map(_.group(2).nn.trim).toSet

    // Pattern 2: `[TAG] ...` — bracketed-tag bare test names (without Suite:: prefix)
    val rxBareBracketed = """`(\[[A-Z][A-Z0-9 -]+\][^`]+)`""".r
    val bareBracketedNames =
      rxBareBracketed.findAllMatchIn(content).map(_.group(1).nn.trim).toSet
  ...
  test(
    "[DOC CITATION] every test-name reference in certificates_strategy.md exists in the test pool"
  ) {
  ...
  test(
    "[DOC CITATION] every test-name reference in certificates_strategy_llm.md exists in the test pool"
  ) {
  ...
  test("[DOC LINKS] every internal link in certificates_strategy.md resolves") {
    val docFile = new File("info/certificates_strategy.md")
    val content = Source.fromFile(docFile, "UTF-8").mkString
    val links = extractLinks(content).filter { case (_, url) =>
      isInternalLink(url)
    }
  ...
  test(
    "[DOC LINKS] every internal link in certificates_strategy_llm.md resolves"
  ) {
    val docFile = new File("info/certificates_strategy_llm.md")
    val content = Source.fromFile(docFile, "UTF-8").mkString
    val links = extractLinks(content).filter { case (_, url) =>
  ...
  test(
    "[ADR] required Phase 9 ADRs (4-7) all exist with both human + LLM copies"
  ) {
    val adrDir = new File("info/adrs")
    val required = Vector(
  ...
  test("[DOC LINKS] info/README.md updates point at certificates_strategy.md") {
    ...
    assert(
      content.contains("certificates_strategy.md"),
