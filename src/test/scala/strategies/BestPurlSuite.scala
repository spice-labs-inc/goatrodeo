...
/** Tests that field-level merge produces pURLs that exist in Maven Central.
  *
  * '''What this tests:'''
  * For JARs where pom.properties is incomplete or absent, the field-level
  * merge algorithm must produce a pURL that can be found in Maven Central as
  * a standalone artifact. This is the "Best" criterion: "Best = can be found
  * in Maven Central."
  *
  * '''Why this matters:'''
  * The entire purpose of field-level merge is to produce pURLs that are more
  * likely to match real Maven artifacts. If the merged pURL doesn't exist in
  * Maven Central, it's not "Best" — it's a fabricated identity.
  *
  * '''How "Maven Central existence" is verified:'''
  * At test time, we hardcode the expected pURLs that we have verified exist
  * in Maven Central (via manual lookup at https://repo1.maven.org/maven2/).
  * No runtime Maven Central queries are performed.
  *
  * '''Test data source:'''
  * The manifest values used in these tests are extracted from real JARs in
  * the test_data/ directory (e.g., commons-codec-1.2.jar inside
  * wps-demo-1.3.0.war). This satisfies HS-4: we use the actual test corpus,
  * not synthetic data.
  *
  * '''LLM Summary:''' This suite verifies the "Best" criterion by checking
  * that field-level merge produces pURLs matching known Maven Central
  * artifacts. Each test calls resolveGAV with realistic manifest/filename
  * combinations and asserts the resulting groupId/artifactId/version matches
  * a real Maven Central coordinate.
  */
class BestPurlSuite extends FunSuite {
  ...
  /** Tests that field-level merge produces a Maven Central pURL for
    ...
    * "org.apache.commons.codec.*", Implementation-Version="1.2", and
    * filename "commons-codec-1.2.jar". Field-level merge should produce
    ...
    * wholesale, producing artifactId="org.apache.commons.codec.*" — a
    * package path with wildcard, NOT a Maven artifactId. The resulting pURL
    ...
    * — confirmed exists.
    *
    * '''Requirement:''' Plan Test 10 — field-level merge produces pURL in
    * Maven Central.
    *
  ...
  test("commons-codec-1.2: field-level merge produces Maven Central pURL") {
    ...
    val manifest = TreeMap[String, TreeSet[StringOrPair]](
      ...
      "implementation-title" -> TreeSet(StringOrPair("org.apache.commons.codec.*")),
    ...
    assertEquals(g, Some("org.apache.commons"), "groupId from manifest Extension-Name parent path")
    // artifactId from filename (field-level merge: filename > manifest for artifactId)
    assertEquals(a, Some("commons-codec"), "artifactId from filename (not manifest Implementation-Title)")
  ...
  /** Tests that field-level merge produces a Maven Central pURL for
    ...
    *
    * '''What it tests:''' A JAR with no pom.properties, manifest with
    * Bundle-SymbolicName="org.apache.commons.lang", Bundle-Version="2.4",
    * and filename "commons-lang-2.4.jar". Field-level merge should produce
    ...
    * — confirmed exists.
    *
    * '''Requirement:''' Plan Test 10 (second case) — field-level merge
    * produces Maven Central pURL for Bundle-SymbolicName case.
    */
  test("commons-lang-2.4: field-level merge produces Maven Central pURL") {
    ...
    assertEquals(a, Some("commons-lang"), "artifactId from filename (not Bundle-SymbolicName)")
