...
/** Tests for the empty-namespace PurlException bug.
  ...
  * `wps-demo-1.3.0.war`), `resolveGAVFromManifest` returns `Some("")` for
  * groupId instead of `None`. The match at `applyAccumulatedAugmentation`
  * line 1096 then treats this as a valid GAV and calls
  * `buildPackageURL(Ecosystems.Maven, Some(""), ...).toCanonical()`, which
  * throws `PurlException: purl type "maven" requires a namespace`.
  *
  ...
  * treated as missing (`None`), not as present-but-empty (`Some("")`). The
  * pURL library rejects empty namespaces for Maven pURLs. The test builds
  * the ADG via `ToProcess.strategyForDirectory` +
  * `ToProcess.buildGraphForToProcess`, which recursively traverses into the
  * WAR and processes each child JAR through its own
  * `applyAccumulatedAugmentation`.
  */
class WpsWarEmptyNamespaceSuite extends FunSuite {

  // ==================== Test 1: resolveGAV unit test ====================

  /** Tests that `resolveGAV` returns `None` for groupId when the manifest has
    * `Implementation-Vendor-Id:` with an empty value.
    *
    * '''What it tests:''' `resolveGAVFromManifest` should treat an empty
    * `Implementation-Vendor-Id` as absent, returning `None` rather than
    * `Some("")`.
    *
    * '''Why:''' An empty string groupId causes `PurlException` when
    * `applyAccumulatedAugmentation` builds a Maven pURL.
    *
    * '''Requirement:''' R1 — `resolveGAV` must never return `Some("")`
    * for any GAV component; empty strings must be normalized to `None`.
    *
    * '''Current behavior (buggy):''' Returns `Some("")` — FAILS until fixed.
    */
  test("resolveGAV returns non-empty groupId when Implementation-Vendor-Id is empty") {
    ...
    assertEquals(a, Some("collections-generic"), "artifactId from Implementation-Title")
    assertEquals(v, Some("4.01"), "version from Implementation-Version")
    assert(g.isDefined, "groupId must be defined (not None) — empty Implementation-Vendor-Id must not produce Some(\"\")")
    assert(g.get.nonEmpty, "groupId must be non-empty — not Some(\"\")")
    assertEquals(g, Some("collections-generic"), "groupId falls back to artifactId when no other source found")
  }
  ...
  /** Tests that building the ADG on `test_data/wps_war_test/` (which contains
    ...
    * value), causing `resolveGAV` to return `Some("")` for groupId, which
    * triggers `PurlException` during pURL construction at Maven.scala:1098.
    *
  ...
  test("ADG build on wps_war_test directory does not throw PurlException") {
    ...
    assert(source.isDirectory, s"Test data not found: ${source.getAbsolutePath}")
    ...
    val hasCollectionsGenericPurl = allPurls.exists(_.contains("collections-generic"))
  ...
  /** Tests that Bundle-SymbolicName provides groupId when
    ...
    *
    * '''What it tests:''' With an empty Implementation-Vendor-Id, the
    * groupId should be derived from Bundle-SymbolicName's parent path.
    * This was previously blocked because `Some("")` short-circuited
    * the `.orElse` chain.
    *
    * '''Requirement:''' R3 — Bundle-SymbolicName fallback for groupId
    * must work when Implementation-Vendor-Id is empty.
    */
  /** Tests that Bundle-SymbolicName provides groupId when
    * Implementation-Vendor-Id is empty, and that artifactId comes from the
    * filename (field-level merge: filename priority 4 > manifest priority 5
    * for artifactId).
    *
    ...
    *
    * '''Requirement:''' R3 — Bundle-SymbolicName fallback for groupId
    * must work when Implementation-Vendor-Id is empty. ArtifactId comes from
    * filename per field-level merge priority.
    *
    * '''Note:''' This test was updated from asserting
    * `a = Some("org.apache.commons.lang")` (OSGi name) to
    * `a = Some("commons-lang")` (Maven artifactId) per the field-level merge
    * plan. The filename-derived value is correct regardless of whether the
    * Maven Bundle Plugin heuristic fires (no `created-by` header in this test).
    */
  test("Bundle-SymbolicName provides groupId when Implementation-Vendor-Id is empty") {
    val state = MavenState()
    val manifest = TreeMap[String, TreeSet[StringOrPair]](
      "implementation-vendor-id" -> TreeSet(StringOrPair("")),
    ...
    assertEquals(g, Some("org.apache.commons"), "groupId from Bundle-SymbolicName parent path")
    assertEquals(a, Some("commons-lang"), "artifactId from filename (Maven artifactId), not Bundle-SymbolicName (OSGi name)")
  ...
  /** Tests that Extension-Name provides groupId when no other source has it.
    ...
    * package-path value (e.g. `org.apache.commons.codec.*`), groupId is
    * derived by stripping `.*` and taking the parent path.
    *
  ...
  test("Extension-Name provides groupId when no other source") {
    ...
    assertEquals(g, Some("org.apache.commons"), "groupId from Extension-Name parent path")
    assertEquals(a, Some("commons-codec"), "artifactId from filename (field-level merge: filename > manifest)")
  ...
  test("Automatic-Module-Name provides groupId when no other source") {
    val state = MavenState()
    val manifest = TreeMap[String, TreeSet[StringOrPair]](
      "automatic-module-name" -> TreeSet(StringOrPair("org.apache.commons.codec")),
    ...
    assertEquals(g, Some("org.apache.commons"), "groupId from Automatic-Module-Name parent path")
  ...
  test("Package header provides groupId when no other source") {
    ...
    assertEquals(g, Some("org.apache.commons"), "groupId from Package parent path")
    assertEquals(a, Some("collections-generic"), "artifactId from Extension-Name")
    assertEquals(v, Some("4.01"))
  }

  // ==================== Test 7: ArtifactId-as-groupId fallback ====================

  /** Tests that resolveGAV uses artifactId as groupId when no groupId source
    * is available.
    *
    * '''What it tests:''' When a JAR has only a filename with no dot-separated
    * prefix (e.g. `collections-generic-4.01.jar`), and no manifest headers
    * with package paths, groupId falls back to the artifactId.
    *
    * '''Requirement:''' R7 — artifactId-as-groupId fallback.
    */
  test("resolveGAV falls back to artifactId as groupId when no groupId found") {
    val state = MavenState()
    val manifest = TreeMap[String, TreeSet[StringOrPair]](
      "implementation-title" -> TreeSet(StringOrPair("collections-generic")),
      "implementation-version" -> TreeSet(StringOrPair("4.01"))
    ...
    assertEquals(g, Some("collections-generic"), "groupId falls back to artifactId")
    assertEquals(a, Some("collections-generic"), "artifactId from Implementation-Title")
    assertEquals(v, Some("4.01"))
  }
  ...
  /** Tests that empty strings in pom.properties are treated as missing,
    ...
    * fromProps level returns None, and resolveGAV falls through to
    * the manifest or filename.
    *
    * '''Requirement:''' R8 — empty strings normalized at each priority level.
    */
  test("resolveGAV normalizes empty strings from pom.properties") {
    val state = MavenState()
    val manifest = TreeMap[String, TreeSet[StringOrPair]](
    ...
    assertEquals(g, Some("com.example"), "groupId from manifest, not empty pom.properties")
  ...
  /** Tests that a pURL with groupId = artifactId is valid (does not throw).
    ...
    * `Some("collections-generic")` as namespace should produce a valid
    * pURL that survives `.toCanonical()`.
    *
  ...
  test("pURL is valid when groupId equals artifactId") {
    val purl = PURLHelpers
      ...
      .toCanonical()
      .nn
    assert(purl.toString.contains("collections-generic"), s"pURL must contain artifactId: ${purl.toString}")
  }

  // ==================== Test 10: Property test — resolveGAV never returns Some("") ====================

  /** Tests that resolveGAV never returns Some("") for any GAV component,
    * regardless of manifest header values.
    *
    * '''What it tests:''' For a variety of manifest configurations including
    * empty values, no component should be Some("").
    *
    * '''Requirement:''' R10 — no Some("") in any GAV component.
    */
  test("resolveGAV never returns Some(empty) for any GAV component") {
    val state = MavenState()
    val manifests = List(
      // Empty Implementation-Vendor-Id
      TreeMap("implementation-vendor-id" -> TreeSet(StringOrPair("")),
      ...
      TreeMap("extension-name" -> TreeSet(StringOrPair("")),
        "implementation-version" -> TreeSet(StringOrPair("1"))),
    )
    for (manifest <- manifests) {
      ...
      assert(g != Some(""), s"groupId must not be Some(\"\") for manifest: $manifest")
      assert(a != Some(""), s"artifactId must not be Some(\"\") for manifest: $manifest")
      assert(v != Some(""), s"version must not be Some(\"\") for manifest: $manifest")
    }
  }
  ...
  /** Tests that after the artifactId-as-groupId fallback, groupId is always
    ...
    */
  test("resolveGAV groupId is Some whenever artifactId is Some") {
    val state = MavenState()
    val manifests = List(
      ...
      TreeMap("implementation-title" -> TreeSet(StringOrPair("my-lib")),
        "implementation-version" -> TreeSet(StringOrPair("1.0"))),
    )
    for (manifest <- manifests) {
      val (g, a, v) = state.resolveGAV(
      ...
      if (a.isDefined) {
        assert(g.isDefined, s"groupId must be Some when artifactId is Some, for manifest: $manifest")
      }
    }
  }
}
