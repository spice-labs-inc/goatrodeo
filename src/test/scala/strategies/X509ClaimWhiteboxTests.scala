...
/** Phase 3-4 white-box tests with **independent ground truth** for
  ...
  * ## Why this exists
  *
  * Phases 3-4 of the Certificates strategy were originally tested primarily
  * through the corpus harness (`CertificatesSuite`), whose sidecars are
  * produced by the `MaterializeSidecars` runner — which uses the strategy's OWN
  * emitters as the source. That makes the per-fixture assertions tautological
  * by construction: a regression in `parseSingleCert` / `parseBundle` /
...
class X509ClaimWhiteboxTests extends FunSuite {
  ...
  test(
    "parseSingleCert + purlsForCert: ISRG Root X1 cert hash matches openssl"
  ) {
  ...
  test(
    "purlsForCert: ISRG Root X1 emits both spki + cert pURLs with openssl-pinned hashes"
  ) {
    val w =
      wrap("test_data/certificates/x509/canonical/letsencrypt-isrgrootx1.pem")
    val cert = Certificates.parseSingleCert(w).get
    ...
    val spkiPurl =
      purls.find(_.name == "spki-sha256").get.toCanonical()
    assert(
      spkiPurl.contains(
        "0b9fa5a59eed715c26c1020c711b4f6ec42d58b0015e14337a39dad301c5afc3"
      )
    ...
    val certPurl =
      purls.find(_.name == "cert-sha256").get.toCanonical()
    assert(
      certPurl.contains(
        "96bcec06264976f37460779acf28c5a7cfe8a3c0aae11a8ffcee05c0bddf08c6"
      )
  ...
  test("parseBundle: goatrodeo-test-chain first-cert hash matches openssl") {
    ...
    assertEquals(
      sha256Hex(firstCert.getEncoded),
      "11c1096ec324244d2dac18cea1131f20981580fb61b50e4cfb1a17f742fa949e",
  ...
  test(
    "parseBundle: walks every BEGIN CERTIFICATE block (count matches openssl)"
  ) {
    val w = wrap(
      "test_data/certificates/pem-bundles/synthetic/goatrodeo-test-chain.pem"
    )
  ...
  test("parseKeystore: trust-only-null-password.p12 loads with null password") {
    val w = wrap(
      "test_data/certificates/keystores/synthetic/trust-only-null-password.p12"
    )
  ...
  test(
    "parseKeystore: trust-only-null-password.p12 alias is letsencrypt-isrg-root-x1 (keytool ground truth)"
  ) {
    val w = wrap(
      "test_data/certificates/keystores/synthetic/trust-only-null-password.p12"
    )
  ...
  test(
    "parseKeystore: trust-only-null-password.p12 cert matches openssl SHA-256 (keytool fingerprint cross-check)"
  ) {
    // keytool -list output:
  ...
  test(
    "parseKeystore: encrypted keystore (null-password load fails) returns Keystore with ks=None"
  ) {
    // The encrypted .p12 fixture is not loadable with null password by
    // design. The strategy must surface envelope-only state.
    val w =
      wrap("test_data/certificates/keystores/synthetic/encrypted-p12-dsa.p12")
    val ks = Certificates.parseKeystore(w, "PKCS12").get
    assertEquals(
      ks.ks,
      None,
