...
/** Strategy-level tests for Phase 7's private-key parsers + emitters.
  ...
  *
  * Phase 7 plan §"Two paths": every Phase-7 fixture must take the
  * correct branch (unencrypted → derive public key → emit pURL +
  * `Envelope=plaintext` + `DerivedFromPrivateKey=true`; encrypted →
  * envelope-only metadata, no pURL, no decryption attempts). The
  * sidecar tests in `CertificatesSuite` are black-box (sidecars
  * materialized from the strategy's own emitters → tautological by
  * construction). These white-box tests pin **independent ground
  * truth** for the PEM fixtures (SPKI SHA-256 computed via `openssl
  * pkey -pubout` + `openssl pkey -pubin -outform DER | sha256sum` —
  * this is the same SHA-256 the strategy must produce).
  *
  * ## Why this matters (HS-3)
  *
  * Phase 6's first-pass remediation (G5) flagged that materializer-
  * sourced sidecar assertions are tautological: a regression in the
  * strategy would re-emit the bug into the sidecar and the test would
  * still pass. Independent ground truth (here: `openssl`'s public-key
  * derivation, run at fixture-creation time) breaks the tautology.
  *
  * ## Ground-truth recipe
  *
  ...
  *
  * For OpenSSH fixtures: ssh-keygen on this system rejects the
  * fixture format (libcrypto mismatch). The OpenSSH tests pin alg
  * + size + structural fingerprint shape; cross-check is via the
  * black-box CertificatesSuite assertions on the materialized sidecar
  * which iterates the entire openssh-key-v1 envelope.
  */
class PrivateKeyStrategyTests extends FunSuite {
  ...
  test("parsePemPrivateKey: PKCS#8 RSA-2048 unencrypted (openssl SPKI ground truth)") {
    // openssl pkey ... | sha256sum: f43c197f37e71d23d15b686d6453947883c79011d766fc6433d58bb9815125c4
    val w = wrap("test_data/certificates/private-keys/synthetic/pkcs8-rsa-2048-unencrypted.pem")
    val claim = Certificates.classifyAndParse(w).get
    val p = claim.asInstanceOf[Certificates.PrivateKeyPlaintextPem]
    assertEquals(Certificates.sha256Hex(p.spkiBytes),
  ...
  test("parsePemPrivateKey: PKCS#8 RSA-3072 unencrypted (openssl SPKI ground truth)") {
    // openssl: 1d69009282c0af2fffb6231037dba363ecde440c9c92e2a297391bbe43e8d3d0
    val w = wrap("test_data/certificates/private-keys/synthetic/pkcs8-rsa-3072-unencrypted.pem")
    val p = Certificates.classifyAndParse(w).get
  ...
  test("parsePemPrivateKey: PKCS#8 RSA-4096 unencrypted (openssl SPKI ground truth)") {
    // openssl: c0612d36fcd581256704fa4cd6d977d51227627f2d0dbd78ba1f7349c0407c48
    val w = wrap("test_data/certificates/private-keys/synthetic/pkcs8-rsa-4096-unencrypted.pem")
    val p = Certificates.classifyAndParse(w).get
      .asInstanceOf[Certificates.PrivateKeyPlaintextPem]
    assertEquals(Certificates.sha256Hex(p.spkiBytes),
  ...
  test("parsePemPrivateKey: PKCS#8 Ed25519 unencrypted (openssl SPKI ground truth)") {
    // openssl: 23061b4699527614aba79f341cccd03865af660ffe4e90b972733a3e5cfd4104
    val w = wrap("test_data/certificates/private-keys/synthetic/pkcs8-ed25519-unencrypted.pem")
    val p = Certificates.classifyAndParse(w).get
      .asInstanceOf[Certificates.PrivateKeyPlaintextPem]
    assertEquals(Certificates.sha256Hex(p.spkiBytes),
  ...
  test("parsePemPrivateKey: PKCS#8 Ed448 unencrypted (openssl SPKI ground truth)") {
    // openssl: c7c43d496c8429929843683134b31303a4be766a617e8f7be582aeba3ac23d6c
    val w = wrap("test_data/certificates/private-keys/synthetic/pkcs8-ed448-unencrypted.pem")
    val p = Certificates.classifyAndParse(w).get
      .asInstanceOf[Certificates.PrivateKeyPlaintextPem]
    assertEquals(Certificates.sha256Hex(p.spkiBytes),
  ...
  test("parsePemPrivateKey: PKCS#8 EC P-256 unencrypted (openssl SPKI ground truth + curve)") {
    // openssl: 3a6e0d4ee737bafcc891b3090e27cf3165de56e6ceac4ba3cd1d02e04265278d
    val w = wrap("test_data/certificates/private-keys/synthetic/pkcs8-ec-p256-unencrypted.pem")
    val p = Certificates.classifyAndParse(w).get
      .asInstanceOf[Certificates.PrivateKeyPlaintextPem]
    assertEquals(Certificates.sha256Hex(p.spkiBytes),
      "3a6e0d4ee737bafcc891b3090e27cf3165de56e6ceac4ba3cd1d02e04265278d")
    assertEquals(p.canonicalAlg, "ec")
    assertEquals(p.curve, Some("p-256"),
      "ECDSA P-256 OID must canonicalize to 'p-256' via ecCurveMap, " +
      "not the BC X962Parameters toString")
  }

  test("parsePemPrivateKey: PKCS#8 EC P-384 unencrypted (openssl SPKI ground truth + curve)") {
    // openssl: 8c8f722d7171d86882b7f399bd92977856eb09f6fce8ea4c7e999a083312ed9e
    val w = wrap("test_data/certificates/private-keys/synthetic/pkcs8-ec-p384-unencrypted.pem")
    val p = Certificates.classifyAndParse(w).get
      .asInstanceOf[Certificates.PrivateKeyPlaintextPem]
    assertEquals(Certificates.sha256Hex(p.spkiBytes),
  ...
  test("parseOpenSshPrivateKey: openssh-ed25519 unencrypted is claimed (alg + envelope)") {
    val w = wrap("test_data/certificates/private-keys/synthetic/openssh-ed25519-unencrypted")
    val p = Certificates.classifyAndParse(w).get
      .asInstanceOf[Certificates.PrivateKeyPlaintextOpenSsh]
    assertEquals(p.algName, "ssh-ed25519")
    assertEquals(p.rsaModulusBits, None)
    assert(p.wireBytes.nonEmpty, "wireBytes must be the in-the-clear public-key blob")
  }
  ...
  test("parseOpenSshPrivateKey: openssh-ed25519 SHA-256 matches Python ground truth (D2)") {
    val w = wrap("test_data/certificates/private-keys/synthetic/openssh-ed25519-unencrypted")
    val p = Certificates.classifyAndParse(w).get
      .asInstanceOf[Certificates.PrivateKeyPlaintextOpenSsh]
    ...
    assertEquals(b64, "oNA+weKy3joG5Lk8DyILmfET8o25s9dl6b7ZwXgZ1Lg",
  ...
  test("parseOpenSshPrivateKey: openssh-rsa-2048 SHA-256 matches Python ground truth (D2)") {
    val w = wrap("test_data/certificates/private-keys/synthetic/openssh-rsa-2048-unencrypted")
    val p = Certificates.classifyAndParse(w).get
      .asInstanceOf[Certificates.PrivateKeyPlaintextOpenSsh]
    val md = java.security.MessageDigest.getInstance("SHA-256")
    val b64 = java.util.Base64.getEncoder.withoutPadding.encodeToString(md.digest(p.wireBytes))
    assertEquals(b64, "t0/kOgTYoKNs5SqVqWSLJPoXV2gsRUIlrprl3Osdlfc")
  }

  test("parseOpenSshPrivateKey: openssh-rsa-4096 SHA-256 matches Python ground truth (D2)") {
    val w = wrap("test_data/certificates/private-keys/synthetic/openssh-rsa-4096-unencrypted")
    val p = Certificates.classifyAndParse(w).get
      .asInstanceOf[Certificates.PrivateKeyPlaintextOpenSsh]
    val md = java.security.MessageDigest.getInstance("SHA-256")
    val b64 = java.util.Base64.getEncoder.withoutPadding.encodeToString(md.digest(p.wireBytes))
  ...
  test("parseOpenSshPrivateKey: openssh-ecdsa-p256 SHA-256 matches Python ground truth (D2)") {
    val w = wrap("test_data/certificates/private-keys/synthetic/openssh-ecdsa-p256-unencrypted")
    val p = Certificates.classifyAndParse(w).get
      .asInstanceOf[Certificates.PrivateKeyPlaintextOpenSsh]
    val md = java.security.MessageDigest.getInstance("SHA-256")
    val b64 = java.util.Base64.getEncoder.withoutPadding.encodeToString(md.digest(p.wireBytes))
  ...
  test("parseOpenSshPrivateKey: openssh-ecdsa-p256 unencrypted (alg + curve via sshAlgMap)") {
    val w = wrap("test_data/certificates/private-keys/synthetic/openssh-ecdsa-p256-unencrypted")
    val p = Certificates.classifyAndParse(w).get
      .asInstanceOf[Certificates.PrivateKeyPlaintextOpenSsh]
  ...
  test("parsePemEncryptedPrivateKey: PKCS#8 AES-256-CBC + PBKDF2 envelope") {
    val w = wrap("test_data/certificates/private-keys/synthetic/pkcs8-encrypted-aes256-pbkdf2.pem")
    val p = Certificates.classifyAndParse(w).get
    ...
    assert(p.kdfIterations.exists(_ > 0L),
  ...
  test("parsePemEncryptedPrivateKey: PKCS#8 AES-128-CBC + PBKDF2 envelope") {
    val w = wrap("test_data/certificates/private-keys/synthetic/pkcs8-encrypted-aes128-pbkdf2.pem")
    val p = Certificates.classifyAndParse(w).get
      .asInstanceOf[Certificates.PrivateKeyEncrypted]
    assertEquals(p.envelope, "pkcs8-encrypted")
    assertEquals(p.kdfAlgorithm, Some("pbkdf2"))
  ...
  test("parsePemEncryptedPrivateKey: PKCS#8 DES-EDE3 + PBKDF2 envelope") {
    val w = wrap("test_data/certificates/private-keys/synthetic/pkcs8-encrypted-des-ede3.pem")
    val p = Certificates.classifyAndParse(w).get
      .asInstanceOf[Certificates.PrivateKeyEncrypted]
    assertEquals(p.envelope, "pkcs8-encrypted")
    assertEquals(p.kdfAlgorithm, Some("pbkdf2"))
  ...
  test("parsePemEncryptedPrivateKey: PKCS#8 scrypt envelope") {
    val w = wrap("test_data/certificates/private-keys/synthetic/pkcs8-encrypted-scrypt.pem")
    val p = Certificates.classifyAndParse(w).get
      .asInstanceOf[Certificates.PrivateKeyEncrypted]
    assertEquals(p.envelope, "pkcs8-encrypted")
  ...
  test("parsePemPrivateKey: legacy PEM `Proc-Type: 4,ENCRYPTED` → pem-legacy-encrypted envelope") {
    val w = wrap("test_data/certificates/private-keys/synthetic/pem-legacy-encrypted-rsa.pem")
    val p = Certificates.classifyAndParse(w).get
      .asInstanceOf[Certificates.PrivateKeyEncrypted]
    assertEquals(p.envelope, "pem-legacy-encrypted")
    assertEquals(p.cipher, Some("aes-256-cbc"),
      "DEK-Info: AES-256-CBC,... → cipher='aes-256-cbc'")
    assertEquals(p.kdfAlgorithm, None,
      "Legacy PEM uses OpenSSL EVP_BytesToKey; no KDF descriptor in the file")
  }

  test("parseOpenSshPrivateKey: encrypted openssh-key-v1 → openssh-encrypted envelope (bcrypt)") {
    val w = wrap("test_data/certificates/private-keys/synthetic/openssh-encrypted-ed25519")
    val p = Certificates.classifyAndParse(w).get
      .asInstanceOf[Certificates.PrivateKeyEncrypted]
    ...
    assert(p.cipher.exists(_.startsWith("aes")),
      s"OpenSSH encrypted defaults to aes256-ctr (or similar); got ${p.cipher}")
  }

  // ===== PGP SECRET KEY (Phase 7 inline) =================================

  test("parsePgpKeyOrSecretKeyRing: unencrypted PGP secret key (Ed25519 + ECDH cv25519 subkey) — gpg ground truth") {
    ...
    assertEquals(p.ring.keys.length, 2,
  ...
  test("parsePgpKeyOrSecretKeyRing: unencrypted PGP secret → derives same pURL as public counterpart") {
    val wSecret = wrap("test_data/certificates/private-keys/synthetic/pgp-secret-ed25519-unencrypted.asc")
    val wPublic = wrap("test_data/certificates/pgp/synthetic/v4-ed25519-pub.asc")
    val secret = Certificates.classifyAndParse(wSecret).get
  ...
  test("[HARD RULE] unencrypted PGP secret derives DerivedFromPrivateKey + Envelope=plaintext metadata") {
    val w = wrap("test_data/certificates/private-keys/synthetic/pgp-secret-ed25519-unencrypted.asc")
    val claim = Certificates.classifyAndParse(w).get
    val state = new CertificatesState(w, Some(claim))
    val (md, _) = state.getMetadata(w, stubItem(),
      io.spicelabs.goatrodeo.omnibor.SingleMarker())
  ...
  test("[HARD RULE] encrypted private key emits NO pURL through CertificatesState") {
    val w = wrap("test_data/certificates/private-keys/synthetic/pkcs8-encrypted-aes256-pbkdf2.pem")
    val claim = Certificates.classifyAndParse(w).get
    val state = new CertificatesState(w, Some(claim))
    val (purls, _) = state.getPurls(w, stubItem(), io.spicelabs.goatrodeo.omnibor.SingleMarker())
    assertEquals(purls, Vector.empty,
      "encrypted private keys must produce zero pURLs (envelope-only path)")
  }

  test("[HARD RULE] unencrypted PKCS#8 emits exactly ONE spki-sha256 pURL with the expected hash") {
    val w = wrap("test_data/certificates/private-keys/synthetic/pkcs8-ed25519-unencrypted.pem")
    val claim = Certificates.classifyAndParse(w).get
    val state = new CertificatesState(w, Some(claim))
    val (purls, _) = state.getPurls(w, stubItem(), io.spicelabs.goatrodeo.omnibor.SingleMarker())
  ...
  test("[HARD RULE] no metadata value matches a forbidden private-key pattern (full corpus sweep)") {
    val pkRoot = java.nio.file.Paths.get("test_data/certificates/private-keys")
    if (java.nio.file.Files.exists(pkRoot)) {
      import scala.jdk.CollectionConverters.*
      val files = java.nio.file.Files.walk(pkRoot).iterator().asScala
      ...
      files.foreach { p =>
        ...
        claimOpt.foreach { claim =>
          ...
          val _ = state.getMetadata(w, stubItem(),
            io.spicelabs.goatrodeo.omnibor.SingleMarker())
        }
      }
    }
