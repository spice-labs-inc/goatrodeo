...
import io.spicelabs.goatrodeo.util.{ArtifactWrapper, ByteWrapper, FileWrapper}
...
import java.security.{KeyPair, KeyPairGenerator, MessageDigest, SecureRandom, Security}
...
/** Phase 8 — generative X.509 roundtrip property tests.
  ...
  * > `X509v3CertificateBuilder`. For each generated cert, assert
  * > [13 properties].
  *
  * The rationale for runtime generation (not corpus-driven): the
  * fixture corpus tests can only check what's in the corpus. A
  * regression in `purlsForCert` that affects an algorithm-key-size
  * combination not present in the corpus would slip through. Runtime
  * generation lets us cover the cartesian product RSA × {2048,3072,
  * 4096} + EC × {p-256,p-384,p-521} + Ed25519 + Ed448 without any
  * fixture additions.
  *
  * Properties are kept STRONG (per the Phase-6 N5 lesson — weak
  * properties pass without testing anything useful). Each property
  * either roundtrips a hash, asserts a structural invariant on the
  * pURL, or guards an absence (no leak).
  */
class CertificatesPropertySuite extends ScalaCheckSuite {
  ...
  /** All algorithm cases the suite generates. Each case knows how to
    * build a `KeyPair` of the right shape. */
  ...
  /** All 8 algorithm cases per the Phase 8 plan: "Generators should
    * cover at minimum: RSA {2048, 3072, 4096}, EC {p-256, p-384,
    * p-521}, Ed25519, Ed448." */
  ...
  /** Random sampling distribution for `forAll`. Used by properties
    * that benefit from variance (parse idempotence, regex shape).
    * Note: with `Gen.oneOf` and 50 runs, statistically every case
    * gets ~6 hits — but no per-run guarantee. The `[STRATIFIED]`
    * tests below give a per-case GUARANTEE per run. */
  private val genCase: Gen[CertGenCase] = Gen.oneOf(allCases)

  /** Stratified sampling: invoke the property block once per
    * algorithm case, guaranteeing all 8 are exercised regardless of
    * ScalaCheck's random seed. Used in tandem with `genCase` when a
    * particular property must hold for every alg class — not just
    * "with high probability over random samples." */
  ...
  private def buildSelfSignedCert(c: CertGenCase): (KeyPair, X509Certificate) = {
    import org.bouncycastle.asn1.x500.X500Name
    import org.bouncycastle.cert.X509v3CertificateBuilder
    import org.bouncycastle.cert.jcajce.{JcaX509CertificateConverter, JcaX509v3CertificateBuilder}
    import org.bouncycastle.operator.jcajce.JcaContentSignerBuilder
    val kp = c.keyPair()
    val subject = new X500Name(s"CN=GoatRodeo Property Test ${c.displayName}, O=GoatRodeo")
    val notBefore = new Date()
    val cal = Calendar.getInstance(); cal.setTime(notBefore); cal.add(Calendar.YEAR, 1)
    ...
    val signer = new JcaContentSignerBuilder(c.signatureAlg).setProvider("BC").build(kp.getPrivate)
  ...
  /** Per the plan, parse the cert through the strategy and check the
    * resulting pURLs. We use `parseSingleCert` + `purlsForCert` to
    * exercise the actual emission path. */
  private def driveThroughStrategy(cert: X509Certificate)
  ...
  property("[PROP] spki-sha256 qualifier matches SHA-256(DER SPKI) computed independently") {
    forAll(genCase) { c =>
  ...
  property("[PROP] cert-sha256 qualifier matches SHA-256(DER cert) computed independently") {
    forAll(genCase) { c =>
      val (_, cert) = buildSelfSignedCert(c)
  ...
  property("[PROP] every key-bearing pURL has exactly one alg= qualifier (not sig-alg)") {
  ...
  property("[PROP] RSA pURLs always carry size= qualifier with positive integer") {
  ...
  property("[PROP] EC pURLs always carry a curve= qualifier with canonical value") {
    val canonicalCurves = Set("p-256", "p-384", "p-521", "secp256k1",
      "brainpoolp256r1", "brainpoolp384r1", "brainpoolp512r1", "curve25519")
    forAll(genCase.suchThat(_.expectedAlg == "ec")) { c =>
      val (_, cert) = buildSelfSignedCert(c)
      val (_, purls) = driveThroughStrategy(cert)
      purls.forall { purl =>
        "curve=([a-z0-9-]+)".r.findFirstMatchIn(purl)
          .map(_.group(1).nn).exists(canonicalCurves.contains)
      }
    }
  }
  ...
  property("[PROP] emitted alg= matches the generator's intended canonical alg") {
    forAll(genCase) { c =>
      val (_, cert) = buildSelfSignedCert(c)
      val (_, purls) = driveThroughStrategy(cert)
      purls.forall { purl =>
  ...
  property("[PROP] every emitted pURL parses through PackageURL without throwing") {
    forAll(genCase) { c =>
      val (_, cert) = buildSelfSignedCert(c)
      val (_, purls) = driveThroughStrategy(cert)
      purls.forall { purl =>
  ...
  property("[PROP] every qualifier value is lowercase / hyphens / digits only") {
    val rxQual = "([a-z-]+)=([^&]+)".r
    forAll(genCase) { c =>
      val (_, cert) = buildSelfSignedCert(c)
      val (_, purls) = driveThroughStrategy(cert)
  ...
  property("[PROP] self-signed certs emit self-signed=true qualifier") {
    forAll(genCase) { c =>
      val (_, cert) = buildSelfSignedCert(c)
      val (_, purls) = driveThroughStrategy(cert)
      purls.find(_.contains("cert-sha256")).exists(_.contains("self-signed=true"))
    }
  }
  ...
  property("[PROP] no emitted metadata value matches any Appendix-C pattern (independent in-test sweep)") {
    forAll(genCase) { c =>
      val (_, cert) = buildSelfSignedCert(c)
      val derBytes = cert.getEncoded
      val artifact: ArtifactWrapper = ByteWrapper(derBytes, "generated.der", None)
      val state = new CertificatesState(
        artifact,
        Some(Certificates.SingleCert(Certificates.parseSingleCert(artifact).get))
      )
      val (md, _) = state.getMetadata(artifact, stubItem(),
  ...
  private val sshCertSidecars: Vector[String] = {
    ...
    if (!java.nio.file.Files.exists(root)) Vector.empty
      ...
      java.nio.file.Files.walk(root).iterator().asScala
        ...
        .filter(p => scala.util.Try(scala.io.Source.fromFile(p.toFile, "UTF-8")
          .mkString.contains("cert-sha256")).getOrElse(false))
    ...
    forAll(genSshCert) { path =>
      ...
      Certificates.classifyAndParse(w) match {
        case Some(c: Certificates.SshCert) =>
          ...
          certPurls.forall { p =>
            val ct = "cert-type=([a-z]+)".r.findFirstMatchIn(p).map(_.group(1).nn)
  ...
  property("[PROP] CRL crl-sha256 hash matches SHA-256(DER CRL bytes) computed independently") {
    forAll(genCrl) { path =>
      val w = FileWrapper(new JFile(path), path, None)
      Certificates.classifyAndParse(w) match {
        case Some(crl: Certificates.Crl) =>
  ...
  perCase("SPKI roundtrip") { c =>
    ...
    val expectedSpkiHex = sha256Hex(Certificates.spkiBytesFromCert(parsed))
    val spkiPurl = purls.find(_.contains("spki-sha256")).get
    assert(spkiPurl.contains(expectedSpkiHex),
  ...
  perCase("emitted alg= matches expected") { c =>
    ...
    val (_, purls) = driveThroughStrategy(cert)
    purls.foreach { purl =>
      assert(purl.contains(s"alg=${c.expectedAlg}"),
