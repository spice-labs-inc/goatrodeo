 ...
 *   - Replace bidirectional `contains` matching with exact match
 ...
 *
 * '''LLM context:'''
 *   The direct unit tests cover `determinePrimaryGroupIdArtifactIdVersion`.
 *   Edge-case unit tests cover empty and short filenameArt values.
 *   Edge-case tests cover version extraction and the separator requirement.
 *   A tie-breaking unit test prefers the longest exact prefix.
 *   Reverse-prefix (score 1) paths are covered by their own tests.
 *   The remaining tests are pipeline-level integration tests.
 *   Property-based tests are in PurlMatchingPropertySuite.
 */

class PurlMatchingSuite extends GoatRodeoFunSuite {
  ...
  // Requirement section:
  //   "Replace with exact match as primary."
  ...
  test("Exact match preferred over substring") {
  ...
  test("Prefix match as fallback when no exact match") {
    val state = MavenState()
    val tuples = Vector(
      ("org.other", "other", "1.0"),
  ...
  // =========================================================================
  // No pURL hijacking via short artifactId
  // =========================================================================
  ...
  test("No pURL hijacking via short artifactId") {
    val state = MavenState()
    val tuples = Vector(
      ("org.evil", "commons", "1.0"),
      ("org.apache.commons", "commons-collections4", "4.4")
  ...
  // Requirement section:
  //   "If no embedded groupId/artifactId/version matches the
  ...
  // =========================================================================
  // Multiple exact matches — first one wins
  // =========================================================================
  ...
  test("Multiple exact matches — first one wins") {
    val state = MavenState()
    val tuples = Vector(
      ("org.first", "foo", "1.0"),
      ("org.second", "foo", "2.0")
  ...
  // =========================================================================
  // Empty filenameArt returns None
  // =========================================================================
  ...
  test("Empty filenameArt returns None") {
    val state = MavenState()
    val tuples = Vector(
      ("org.example", "foo", "1.0")
    )
  ...
  // =========================================================================
  // Short filenameArt (< 2 chars) returns None
  // =========================================================================
  ...
  test("Single-character filenameArt returns None") {
    val state = MavenState()
    val tuples = Vector(
      ("org.example", "a", "1.0")
    )
  ...
  // =========================================================================
  // extractIdentityFromFilename strips version correctly
  // =========================================================================
  ...
  test("extractIdentityFromFilename strips version correctly") {
    val state = MavenState()
    // Standard version
    val (g1, a1, v1) =
  ...
  // =========================================================================
  // Prefix match requires separator after matched prefix
  // =========================================================================
  ...
  test("Prefix match requires separator after matched prefix") {
    val state = MavenState()
    val tuples = Vector(
      ("org.spring", "spring", "5.3")
    )
  ...
  test("Longest prefix match preferred") {
    val state = MavenState()
    val tuples = Vector(
      ("org.spring", "spring", "5.3"),
  ...
  // =========================================================================
  // Reverse prefix match — filename shorter than artifactId
  // =========================================================================
  ...
  // Requirement section:
  //   reverse prefix match (priority 3).
  ...
  test("Reverse prefix match — filename shorter than artifactId") {
    val state = MavenState()
    val tuples = Vector(
      ("org.other", "other", "1.0"),
  ...
  // =========================================================================
  // Reverse prefix without separator returns None
  // =========================================================================
  ...
  // Requirement section:
  //   separator requirement for all prefix matches.
  ...
  test("Reverse prefix without separator returns None") {
    val state = MavenState()
    val tuples = Vector(
  ...
  // =========================================================================
  // Pipeline-level — no pURL hijacking via short artifactId
  // =========================================================================
  ...
  test("Pipeline — no pURL hijacking via short artifactId") {
  ...
  // =========================================================================
  // Pipeline — filenameArt extraction uses extractIdentityFromFilename
  // =========================================================================
  ...
  test(
    "Pipeline — filenameArt extraction correct for dotted versions"
  ) {
...
/* Property-based tests for pURL matching.
 *
 * '''What this suite tests:'''
 *   - For any set of tuples where one exactly matches
 *     the filename, the exact match is always selected.
 *   - For any pair where one artifactId is a substring of
 *     another, the longer (more specific) one is preferred when it
 *     matches the filename.
 */
class PurlMatchingPropertySuite extends GoatRodeoScalaCheckSuite {
