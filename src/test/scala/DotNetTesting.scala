...
test("Can build for a simple dotnet file") {
  ...
  test("assembly-references") {
    ...
    assertEquals(deps, Some("{\"dependencies\":[{\"name\":\"System.Console\",\"version\":\"9.0.0.0\",\"public_key_token\":\"b03f5f7f11d50a3a\"},{\"name\":\"System.Runtime\",\"version\":\"9.0.0.0\",\"public_key_token\":\"b03f5f7f11d50a3a\"}]}"))
  }

  test("Can get purl from dll") {
    val name = "test_data/hackproj.dll"
    val wrapper = FileWrapper(File(name), name, None)
    val store1 = ToProcess.buildGraphFromArtifactWrapper(wrapper, args = Config())
