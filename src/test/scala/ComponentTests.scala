...
class PurlComponent extends RodeoComponent {
    ...
    override def importAPIFactories(factorySource: APIFactorySource): Unit = {
        val purlAPIFactory = factorySource.getAPIFactory(PurlAPIConstants.NAME, this, classOf[PurlAPI]).toScala
        purlAPI = purlAPIFactory.map(paf => paf.buildAPI(this))
    }
    override def onLoadingComplete(): Unit = {
        purlAPI.map(api => {
            val factory = api.newPurlFactory().withType("maven").withName("rodeo-components").withNamespace("spice-labs")
            try {
                val purlAdapter = factory.toPurl().asInstanceOf[PurlAdapter]
                val stringRep = purlAdapter.purl.toString()
                PurlComponent.correctPurl = stringRep == "pkg:maven/spice-labs/rodeo-components"
            }
            catch {
                case badURL: MalformedURLException => PurlComponent.exMessage = badURL.getMessage()
