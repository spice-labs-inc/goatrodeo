...
/** Base class for Goat Rodeo test suites.
  *
  * Goat Rodeo's integration tests are CPU-bound and workload-bound (large
  * real corpora, full build pipelines), and they share runners with other
  ...
  * CPU-constrained CI runners. Per the project decision, every test carries
  * a uniform 60-minute budget: the cap is a safety net against hangs, NOT
  * an acceptance criterion. Tests must pass — or fail on their assertions —
  * never be killed by the clock on a busy runner.
  *
  * LLM note: all Goat Rodeo suites extend one of the two base classes in
  * this file; there is no other place where a per-test timeout is set.
  */
