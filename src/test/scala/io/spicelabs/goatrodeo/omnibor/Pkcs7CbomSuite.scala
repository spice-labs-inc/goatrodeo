...
/** PKCS#7 certificates surface in CBOM .
  ...
  * component with `assetType: certificate`, certificateProperties
  * (subjectName/issuerName/dates/format/refs), and per-cert + bundle metadata.
  * Also: component-equivalence vs a PEM bundle carrying the same certificate,
  * and the negative (an invalid blob never appears as a certificate component).
  *
  * WHY: the spec requires the PKCS#7 claim's certs to surface in the CBOM
  * exactly like a PEM bundle. `hasCertificate` already keys on
  * `Certificates:Cert:` metadata, so the claim routing via Bundle must exercise
...
class Pkcs7CbomSuite extends GoatRodeoFunSuite {
  ...
  private def emitFor(
    ...
    val files = CbomEmitter
      .emitForStorage(store, "1.6", outDir)
      .toOption
  ...
  test("pkcs7CertAppearsInCbom") {
    ...
    assert(
      cbom.contains("certificate"),
      s"cbom must contain assetType certificate:\n$cbom"
    )
    assert(
      cbom.contains("GoatRodeo Test Root"),
      s"cbom must carry the cert subject:\n$cbom"
    )
    assert(
      cbom.contains("X.509"),
      s"cbom must carry certificateFormat X.509:\n$cbom"
    )
  }
  ...
  test("bundleMetadataRecorded") {
    ...
    assert(
      ...
      s"bundle EntryCount must appear:\n$cbom"
    )
    assert(
      cbom.contains("Certificates:CertCount"),
      s"bundle CertCount must appear:\n$cbom"
    )
    assert(
      cbom.contains("Certificates:Cert:0:"),
      s"per-cert metadata must appear:\n$cbom"
    )
  }
  ...
  test("cbomComponentEquivalencePkcs7VsPem") {
    ...
    assert(
      p7.contains("O=Spice Labs,CN=GoatRodeo Test Root") &&
  ...
  test("invalidBlobNeverInCbomAsCertificate") {
    ...
    assert(
      !cbom.contains("assetType\":\"certificate"),
      s"no certificate asset allowed:\n$cbom"
    )
    assert(
      !cbom.contains("Certificates:Cert:0:"),
      s"no per-cert metadata allowed:\n$cbom"
    )
  }
}
