...
/** Phase 3 — git provenance Items are NOT CBOM inputs (spec §6 × CBOM docs; the
  ...
  * emitter's crypto-detection set never matches them, and the emitter's
  * traversal starts at artifact roots (isRoot excludes `bodyMimeType !=
  * application/vnd.cc.goatrodeo`…), so git Items can never surface as
  * components.
  *
  * WHY: downstream CBOM builders must ignore git provenance nodes; the docs
  * state this, and the test proves it.
  *
...
class GitProvenanceNotInCbomSuite extends FunSuite {
  ...
  test("git provenance items are ItemTagData, never crypto items") {
    ...
    items.foreach { gi =>
      assert(
        gi.gitoid.startsWith("gitoid:commit:sha1:") || gi.gitoid.startsWith(
          "gitoid:tree:sha1:"
        )
      )
