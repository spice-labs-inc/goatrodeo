...
/** Certificates PKCS#7 claim + parse .
  ...
  * (Authenticode shape) via CertificateFactory into the embedded X.509 chain,
  * records per-cert and bundle metadata like a PEM bundle, does NOT claim the
  * PEM/CMS MIME, never mislabels invalid blobs, and keeps the cert MIME
...
class CertificatesPkcs7Suite extends GoatRodeoFunSuite {
  ...
  test("bareDerSingleCertSharesPath") {
    ...
    parsed.foreach {
      case Certificates.SingleCert(c) =>
        assert(c.getSubjectX500Principal.getName.nonEmpty)
  ...
  test("invalidBlobNeverClaimed") {
    ...
    assertEquals(
      parse(w),
      None,
      "unparseable blob must skip cleanly, not throw"
    )
  }
