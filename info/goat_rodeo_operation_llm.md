...
a single item has taken longer than 30 seconds (the only per-item attribution
line), and once more with the final count when a batch drains. A run that
finishes quickly therefore produces one terminal progress event with
`current == total` and no mid-run noise. Hosts synthesizing phases from
events can rely on a `current == total` event meaning the run's own
processing is done (as demonstrated by `ProgressListenerIntegrationTest` "the
batch-drain final report carries the final count" and "Howdy.run delivers
monotonic progress to a ProgressListener").

The initial-setup pass ("Initial file setup") reports its first line
immediately and then at most once per 30 seconds, so a fast machine sees one
line and a slow machine sees progress (no test pins the timing leg; the
behavior is deliberately time-based and untestable without sleeping).
...
incremented. The count is what the completion reporting aggregates — items
inside a failed container are never processed while the ingested list still
grows, so the counts alone cannot show the loss. The counting contract is
pinned by `ISOFileSuite` "withinArchiveStream - counts a container whose walk
fails"; the completion line itself is the thin reporting glue over that
count.
