...
2. `sbt osvCheck` runs the gate entirely inside the build tool (no
   Python, no shell scripts — the build's `osvCheck` task, implemented
   by `io.spicelabs.goatrodeo.util.OsvGate`, is compiled with the
   product and invoked by the task). It filters to the product's
   configurations (compile, compile-internal, test, test-internal,
   runtime, runtime-internal, provided, provided-internal), excludes
...
3. The gate prints a JSON summary (queries, findings, unscored, high)
   and ends with:
   - PASS — no finding with a concrete score ≥ 7.0
   - FAIL — at least one finding with a concrete score ≥ 7.0 (the task
     throws so CI fails)
   - INFRA — infrastructure failure (transport error or malformed
     response; the task throws so CI alerts; never a silent pass)

## Semantics

- **Fail-open on unscored advisories.** An advisory without a resolvable
  CVSS score is reported (warn) and never fails the gate.
...
- **Independent CI job.** The `osv` job in
  ...
  job (no `needs:`) and runs `sbt osvCheck`.
